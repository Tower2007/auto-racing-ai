...
# 2026-07-12 Codex艦隊監査 P2-2: 購入ゲートは backstop_blocks_purchase()
# (履歴読取失敗時 fail-closed = 三連系購入スキップ) に変更。
from src.backstop import backstop_blocks_purchase, enforce_backstop  # noqa: E402
RT3_THR = 1.80
...
def main():
    ...
    # 三連系 (メールの 3 点購入ボタン / auto_buy) が止まる。
    # 2026-07-12 P2-2: 履歴読取失敗時も rt3_buy_active() 側が fail-closed で
    # 三連系購入をスキップする (sticky フラグは真の発動時のみ書く)。
    try:
