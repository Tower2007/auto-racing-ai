...
#   LEAD_MIN=3 → 発火から締切まで 30 秒のみ。Playwright 実行は 28〜35s (複勝 1点)
#   のため、predict 処理が極めて速い場合のみ滑り込める綱渡り状態。
#   余裕を持たせるなら LEAD_MIN=4 (締切まで 90s) を推奨。
# 3 分前 (2026-06-05〜): 締切前マージン確保(最低限)。
LEAD_MIN = 3
RACES_PER_DAY = 12
