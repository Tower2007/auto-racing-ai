...
# 2 分前 (2026-06-04〜): auto_buy 実測値 28〜57s、2分前発火で発走 1分前に注文確定。
# オッズ鮮度最優先。セッション切れ時の auto_login (~90s) だけがリスクだが
# 朝の fetch_order_history で session を保温しているため実用上問題なし。
LEAD_MIN = 2
RACES_PER_DAY = 12
