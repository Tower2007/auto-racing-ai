...
> 旧 meta (n_train+n_val=262,476) と現行 parquet (264,304) で同じ日付範囲
> (<=2026-04-26) なのに +1,828 行ある。is_absent 変化が原因か?

### 調査結果: is_absent ではなく finished フラグのロジック変更が原因

commit `5954804` (2026-05-02) で `ml/features.py` の `_build_target()` を変更:

| 項目 | 旧 | 新 |
|---|---|---|
| finished の定義 | `order.notna()` | `1` (race_results に行がある = 全て finished) |
| 影響 | DQ/落車 (order=NaN) は finished=0 で除外 | DQ/落車も finished=1 で学習負例に含まれる |

**検算**:
- 非欠場で order=NaN (DQ/落車) の行数 = **1,828**
- 264,304 (現行) - 1,828 = **262,476** (旧 meta) → 完全一致
- is_absent 列自体は変化なし (現行 3,805 = 旧推定 5,633 - 1,828)

**is_absent の内訳整理**:
- 旧 meta で推定した「旧 absent 5,633」= is_absent=1 (3,805) + order=NaN (1,828)
  - つまり旧ロジックでは「欠場」と「DQ/落車」が両方 prepare フィルタで除外されていた
- 新ロジックでは DQ/落車が学習データに含まれるようになった (意図的な変更)

**DQ/落車の分布**: 全体の約 1%、年による偏りなし (2021: 0.93% ~ 2026: 1.16%)

### 再学習実験: finished 変更が best_iter 低下の主因と確定

Codex 提案の「現行 parquet を 04-26 で切って旧条件再学習」を実施。
同一データ範囲 (<=2026-04-26)、同一パラメータで新旧条件を比較:
...
| train_auc | 0.8325 | 0.8254 | 0.8325 |
| valid_auc | 0.8266 | 0.8249 | 0.8266 |
| valid_logloss | 0.49901 | 0.50057 | 0.49901 |

**旧条件は 4/29 meta と全指標が完全一致**。新条件では best_iter が 271→210 に低下。
DQ/落車 1,828 行 (全体の 0.7%) の追加が early stopping を早期発火させている。

5/10 再学習ではさらに新データ (4/27~5/10) が加わり 210→143 まで下がったと推定。
つまり best_iter 半減の root cause は **finished フラグ変更 + 新データの複合効果**。

### Codex への相談: DQ/落車の扱いをどうすべきか

DQ/落車を負例に含めること自体は学習理論的に正しい (出走した結果として失格=3着以内
ではない)。しかし実測では:

- best_iter 低下 (271→210、新データ追加でさらに 143 へ)
- valid_auc 低下 (0.8266→0.8249)
- valid_logloss 悪化 (0.49901→0.50057)

DQ/落車は「通常の負け」とは異なるノイジーな事象 (落車はランダム性が高い) なので、
学習を「通常の着順予測」から逸脱させている可能性がある。

**選択肢**:
1. **revert (旧条件に戻す)**: finished = order.notna() に戻して DQ/落車を除外。
   best_iter 271 に復帰し、4/29 モデルと同条件で週次再学習が安定して回る。
2. **新条件を維持して調整**: DQ/落車を含めたまま early_stopping_rounds を
   50→100 に増やす等の調整で best_iter 低下を緩和する。
3. **DQ/落車に専用 target (例: target_top3=NaN) を割り当てる**: 学習から除外せず、
   ただし target を不定として LightGBM に「この行の label は信用しないで」と伝える
   (LightGBM の NaN label 対応次第)。

Claude の推奨は **選択肢 1 (revert)**。理由:
- 実測で明確に品質低下している
- DQ/落車は全体の 0.7% で、含めても除外しても予測性能への寄与は軽微
- 4/29 モデルと同条件を維持することで品質ゲートの基準が安定する
- 「理論的に正しい」改善は、実測で品質向上が確認できてから採用すべき

ただし Codex の意見を聞いてから最終判断したい。

---

## 2026-05-14: 直近成績悪化の原因分析 + モデルロールバック + 品質ゲート導入

### 観測事実

直近 7 日 (5/7〜5/12) の成績が急落。

| 期間 | 賭数 | 的中率 | P&L | ROI |
|---|---|---|---|---|
| 4/29〜5/6 (旧モデル) | 49 R | 78% | +¥1,590 | 132% |
| 5/7〜5/12 (含 新モデル) | 35 R | 54% | -¥660 | 81% |

95% 信頼区間 [57.9%, 77.8%] の下限を下回っており、統計的に有意な悪化。

### 原因分析 (3 軸)

**① モデル再学習 (5/10)**
- 旧: valid_auc=0.8266, best_iter=271
- 新: valid_auc=0.8242, best_iter=143
- AUC 低下 + best_iteration が半減 → 学習が浅すぎた可能性大
- 再学習後 (5/10〜) は 21 R 中 11 hit (52%), ROI 87.6%

**② Odds Drift (構造的問題、主犯)**
- 発火時 → 確定時のオッズ変動: 平均 **-19.6%**、中央値 **-28.6%**
- 全 84 件中 59 件 (70%) でオッズ低下
- 発火時 EV mean=2.32 → 確定時 EV mean=1.57
- **確定時に EV < 1.50 (本来なら候補外): 48/84 = 57%**
- 特に悪い日: 5/9 drift -30.7%, 5/11 drift -28.1%

**③ 場別 (直近 7 日)**
- 浜松: 0 勝 3 敗 (全滅)
- 川口: 8/15 hit (53%), P&L -250

### 対応

1. **モデルロールバック** (実施済): 5/10 新モデル → 4/29 旧モデルに復元
   - `git show 03fdf4d:data/production_model.lgb` で git から復元
2. **品質ゲート導入** (実施済): `ml/train_production.py` に `_should_adopt()` を追加
   - valid_auc 低下 → 見送り
   - valid_logloss 1% 以上悪化 → 見送り
   - best_iteration が旧の 40% 未満 → 見送り
   - 見送り時は `production_meta.rejected.json` に記録、旧モデル維持
   - `--force` で強制採用可
3. **Odds Drift 対策は未着手** — 構造的問題として今後検討が必要
   - 案 A: EV 閾値引き上げ (1.50 → 2.00)
   - 案 B: 過去平均 drift を織り込んだ補正
   - 案 C: 確定オッズ近似 (直前 2 分のオッズ推移から最終値を予測)

### Codex への相談事項

- Odds drift 対策として A/B/C どれが妥当と思うか？他に良い案があれば提案してほしい
- 品質ゲートの閾値 (AUC 低下=即 NG、logloss +1%、best_iter 40%) は妥当か？
  もう少し緩く/厳しくすべきか意見がほしい
- 5/10 再学習で best_iter が 271→143 に激減した root cause の仮説があれば知りたい
  (データ量増? ノイズ? ハイパーパラメータ?)
...
## 2026-05-08: click-to-buy 機構の導入(完全自動売買とは別物として採用)

ユーザー提案「メールに『購入する』ボタンを追加して、そこ押すと推奨通りに
購入するスクリプトを実行させる」を **「自動売買とは別物」** として採用。

### 議題化判断

memory `ml_baseline_findings.md` 2026-05-05 に記録した「自動売買は議題化しない」
ルールがあるが、**今回の提案は完全自動売買とは構造的に異なる** ため蒸し返しでは
ないと判断:

| 軸 | 完全自動売買 (不採用維持) | click-to-buy (今回採用) |
|---|---|---|
| 意思決定 | scheduled / autonomous | 本人 click が毎回必要 |
| 第13条「自ら申込む」 | 解釈アウト | セーフ寄り(各 click = 意思表示) |
| 拡大可能性 | 金額・券種・回数 制限が緩い | 金額・券種・回数 全部 server-side で固定 |

### 実装した制限(構造的な「線」)

ユーザーが事後的に「やっぱり拡大したい」となっても、**コード変更しないと
拡大できない** ように設計:
- `MAX_AMOUNT_YEN = 1000` を execute_purchase.py に hard-code
- 複勝以外 = vel_code mapping で間接的に制限(他券種の selector 未実装)
- 1 token 1 回限り = buy_tokens.csv で is_consumed チェック

### dry-run first 戦略

実購入の selector(form 入力・確認モーダル等)は **inspect_purchase_page.py
で要調査**。初期 commit は dry-run only(navigate のみ動作)。これを最初に
回してから、ユーザー目視 + 私の selector 調整 → 慎重に本番 enable する流れ。

これは IPO project の pattern(初回 device_auth で目視 → 以降 自動化)を踏襲。

### Phase 4 不採用の線

「完全自動(click なし)」は引き続き不採用。今後同じ提案が来ても以下を引用:
- 金額・券種・回数の構造的制限が崩れる(scheduled になると click ごとの
  制限が無くなる)
- 「自ら申込む」の解釈が再びアウト寄りに戻る
- AML / proxy 懸念が再発

### 自分の慎重論

実装中ずっと意識した点:
- accounts.json の buy_secret_key を平文で保存することの是非 → IPO project
  と同じ pattern なので許容、ただし NTFS 権限が前提
- LAN 内のみアクセス可 = 外部攻撃面はゼロ(LAN IP を URL に直書きするので、
  ファイアウォールで LAN 外からはアクセス不可)
- HMAC token = 改ざん検出、24h 期限切れ、1 回限り = 中間者攻撃にも耐性
- 「初回は dry-run」と必ず通る fail-safe デザイン

最大のリスクは **「規約違反判定でアカウント停止」**。これは技術ではなく
ユーザー自己責任の領域、私からは memory に明記した上で実装した。
