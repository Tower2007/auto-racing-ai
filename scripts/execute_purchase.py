...
def _deme_str(bet_type: str, cars: list) -> str:
    """券種別の packDeme 表記 (表示用)。fns='6' / rt3='6-5-4' / rf3='4=5=6'(昇順)。"""
...
def _deme_match(bet_type: str, exp_cars: list, actual_deme: str) -> bool:
    ...
    車番は 1-8 の 1 桁なので packDeme から数字を抽出して比較する。
    - fns: 単一車番一致
    - rt3: 着順込みで数字列が一致 (例 actual '5-6-7' → [5,6,7])
    - rf3: 順不同なので集合一致 (BOX 投票時 actual は '567=567=567' の
           BOX 表記になるため、集合で比較する。2026-05-30 本番で確定)
    """
    import re as _re
    digits = [int(d) for d in _re.findall(r"\d", str(actual_deme))]
    exp = [int(c) for c in exp_cars]
    if bet_type == "fns":
        return digits == exp
    if bet_type == "rt3":
        return digits == exp
    if bet_type == "rf3":
        return set(digits) == set(exp)
    return False
                    ...
                    for p in o.get("packs", []):
                        ...
                        for exp in expected:
                            ...
                            if (bet_type == exp["gql_type"]
                                    and _deme_match(exp["type"], exp["cars"], deme)
