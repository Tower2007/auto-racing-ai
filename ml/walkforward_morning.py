"""中間モデル(朝モデル): 試走情報なし・オッズあり

試走タイム関連だけを除外、オッズ系は使う。
朝バッチで EV 確定通知ができる前提のモデル。
"""
...
def prepare(df: pd.DataFrame, target_col: str) -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    logger.info("Morning feature count: %d (excluded %d trial-related)",
                X.shape[1], len(MORNING_EXCLUDE))
    return X, y, df


def _train(X_tr, y_tr, X_va, y_va) -> lgb.Booster:
...
def main():
    ...
    logger.info("Test months=%d", len(test_months))
    ...
    for i, tm in enumerate(test_months, 1):
        ...
        model = _train(X_tr, y_tr, X_va, y_va)
        ...
        logger.info("[%2d/%d] %s | n=%5d auc=%.4f best_iter=%d",
                    i, len(test_months), tm, len(X_te), auc, model.best_iteration)

    pred_df = pd.concat(pred_frames, ignore_index=True)
    out = DATA / f"walkforward_predictions_morning_{args.target}.parquet"
