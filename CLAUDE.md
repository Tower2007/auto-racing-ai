...
| `AutoraceOddsTsCollector` | 毎日 07:05 | `wscript //B scripts/run_odds_ts_hidden.vbs` → `scripts/odds_timeseries_collector.py`: 当日全レースのオッズを発走 -60/-30/-15/-8/-4/-3/+2 分の 7 時点でスナップショット、全券種 body を `data/odds_ts/YYYY-MM-DD.jsonl` に追記 (+2分 = 締切後の確定オッズ = 教師データ)。将来の確定オッズ予測 / smart money 検知 / 三連単順序モデル研究用の観測データ蓄積 (2026-07-05 導入)。読み取り専用・投票と無関係。再起動安全、最終レース+2分で自動終了 |
