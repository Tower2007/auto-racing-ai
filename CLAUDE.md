...
| `AutoraceWinNotify` | 毎日 02:45 | `wscript //B scripts/run_win_notify_hidden.vbs` → `scripts/win_notify.py`: `AutoraceFetchOrderHistory`(02:30) 直後に前日の**的中**を集計し「🎯 当たりました」メール送信。**的中ゼロの日は送らない**(ハズレ通知はノイズ)。従来は投票完了通知しか無く「当たった実感が無い」問題への対応 (2026-07-18 導入) |
