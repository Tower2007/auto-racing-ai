...
| `AutoraceNightReport` | 毎日 22:00 | `wscript //B scripts/run_night_report_hidden.vbs` → `night_report.py`: **夜次メール** (夜次メール契約 v1、統合監視 `docs/nightly_mail_reflection_spec.md`)。当日実発注 (auto_buy_state) を RaceRefund API で暫定精算 + 前日確定 (bet_history) + 累計 + 振り返り 1 節をメール。**反省の正本は `logs/reflections/YYYY-MM.md`** (契約 §3、メール本文だけに存在する反省は禁止)。発注ゼロ・前日確定なしの日は送らない。未確定レースは「結果待ち」表示し翌晩の前日確定で補正 (boat と同じ2段構え)。週次 (weekly_status) の「今週の反省まとめ」節が転記して読み返す (契約 §5) (2026-08-07 導入) |
