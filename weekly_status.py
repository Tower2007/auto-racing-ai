...
# kill-switch は「現役ポリシーの健全性」を判定する (選択肢 B, 2026-07-11)。
# 監視スコープ = 現在の購入対象 (src/strategy_config が正本)。購入対象を変えれば
# 監視も自動追従するため、監査 P1-3 のようなスコープ desync が構造的に起きない。
# 廃止した場 (例: 飯塚) の過去損失は全体 P&L / 資金管理には残るが、現役ポリシーの
# ROI・n・停止基準には持ち込まない (過去の廃止判断を現在の停止条件に混ぜない)。
# 2026-07-11 Codex 追加指摘: 現役の集計は場×券種の「正確なペア」で絞る。
# 「現役場の rt3/rf3 全て」だと廃止済みの組 (例: 伊勢崎 rt3) の損益が混入する。
from src.strategy_config import (  # noqa: E402
    THREE_POINT_BUY_PLACES, THREE_POINT_POLICY_PAIRS,
)
from src.backstop import enforce_backstop  # noqa: E402  (条件⑤ バックストップ)
RT3_PLACES = tuple(sorted(THREE_POINT_BUY_PLACES))   # 現在: (3, 4, 6) 伊勢崎・浜松・山陽
...
def check_3point_health() -> dict:
    """三連系まとめ買いの機械的停止基準を評価 (現役ポリシー健全性, 選択肢 B)。

    評価対象は現役ポリシーの正確な (place_code, bet_type_code) ペア
    (THREE_POINT_POLICY_PAIRS が正本) のみ。廃止した場・券種の過去損失は
    全体 P&L には残るが、この健全性判定には含めない (2026-07-11 Codex 指摘で
    「現役場の rt3/rf3 全て」→「場×券種の厳密一致」に修正。伊勢崎 rt3 等の
    廃止済みペアの損益混入を排除)。
    ...
    停止条件 (docs/ev_strategy_findings.md 2026-05-31):
      ...
      ⑤ 全場バックストップ: 全場・全期間の累積損失 ≤ -¥10,000 (sticky、
         ポリシー変更でリセットされない。正本: src/backstop.py、2026-07-11 監査 P2)
    ①② を concrete 評価し、いずれか発動で stop。③④ は n / データ不足時は「未評価」。
    ⑤ は独立フラグ (data/rt3_backstop_stop.flag) で停止し、人間の削除でのみ解除。
    ...
    # ⑤ 全場・全期間 絶対損失バックストップ (sticky)。detail の有無より先に評価
    # (フラグ有効中は detail が読めなくても停止中である事実を表示する)。
    # 新規発動時は enforce_backstop() がフラグ書き出し + メール通知まで行う。
    try:
        bs = enforce_backstop(notify=True)
        out["backstop"] = bs
        thr = bs.get("threshold")
        profit_all = bs.get("profit")
        if bs.get("active"):
            out["conditions"].append((
                "⑤", "NG",
                f"全場バックストップ発動中 (sticky): 全場累積 "
                + (f"¥{profit_all:,}" if profit_all is not None else "—")
                + f" ≤ ¥{thr:,} — 解除は data/rt3_backstop_stop.flag を人間が削除"))
        elif profit_all is not None:
            out["conditions"].append((
                "⑤", "OK",
                f"全場バックストップ OK: 全場・全期間累積 ¥{profit_all:,} "
                f"> ¥{thr:,}"))
        else:
            out["conditions"].append((
                "⑤", "INFO",
                f"全場バックストップ 未評価: {bs.get('error') or 'データなし'}"))
    except Exception as e:
        out["conditions"].append(("⑤", "INFO", f"全場バックストップ評価失敗: {e}"))
    ...
    # 現役ポリシーの (place_code, bet_type_code) 厳密一致で絞る (Codex 2026-07-11)。
    # 場単位フィルタだと廃止済みペア (例: 伊勢崎 rt3) の損益が現役に混入する。
    pair_ok = [
        (int(pc), str(bt)) in THREE_POINT_POLICY_PAIRS
        for pc, bt in zip(df["place_code"], df["bet_type_code"])
    ]
    sub = df[pd.Series(pair_ok, index=df.index)].copy()
    if sub.empty:
...
def render_3point_text(tp: dict) -> str:
    if not tp:
        return ""
    lines = ["", "【🎯 三連系まとめ買い 停止基準監視 (現役スコープ rt3+rf3)】"]
    if tp.get("flag_exists"):
        lines.append("  🛑 停止フラグ ON (data/rt3_stop.flag) — 三連系購入は現在停止中")
    if (tp.get("backstop") or {}).get("active"):
        lines.append("  🛑 全場バックストップ ON (data/rt3_backstop_stop.flag, sticky) "
                     "— 三連系購入は停止中 (解除は人間がフラグ削除)")
    roi = tp.get("roi")
    lines.append(
...
def render_3point_html(tp: dict) -> str:
    ...
    if (tp.get("backstop") or {}).get("active"):
        flag_banner += (
            '<p style="color:#c62828; font-weight:bold; margin:4px 0;">'
            '🛑 全場バックストップ ON (data/rt3_backstop_stop.flag, sticky) — '
            '三連系購入は停止中 (解除は人間がフラグ削除)</p>')
    rows = []
    for cid, level, msg in tp.get("conditions", []):
